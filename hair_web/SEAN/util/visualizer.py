...
class Visualizer():
    ...
    def save_images(self, visuals, image_path,result_dir):        
        visuals = self.convert_visuals_to_numpy(visuals)        
        
        for label, image_numpy in visuals.items():
            save_path = os.path.join(result_dir, '{}.png'.format(label))
            print(save_path)
            util.save_image(image_numpy, save_path, create_dir=False)
